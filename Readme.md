...
├── can/                 # CAN wrapper source code
├── common/              # Shared headers and utilities
├── docs/                # Techstream reference materials
├── isotp/               # ISO-TP handling
├── mcp_can/             # MCP2515 interface
├── services/            # Diagnostic services
├── techstream_uds_logs/ # Ground truth logs
...
| File Name                                           | Purpose                                                                                                                                                                                                  |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decoding Techstream canbus messages.md**          | Explains the logic for decoding Diagnostic Trouble Codes (DTCs) for Engine and ABS ECUs using sample CAN messages and byte-by-byte breakdowns. Useful for understanding DTC extraction and status logic. |
| **LexusI ISF Canbus Message Definitions.json**      | The authoritative JSON mapping of Lexus ISF CAN bus messages, including parameter names, scaling, byte offsets, and UDS identifiers. This is the ground truth for message structure and decoding.        |
| **Techstream.md**                                   | Overview of Toyota/Lexus Techstream diagnostics, including UDS/ISO-TP usage, common SIDs, and request/response formats. Essential for understanding how Techstream communicates with ECUs.               |
| **Understanding CAN Bus Messages in Techstream.md** | High-level guide to interpreting CAN bus messages captured from Techstream, including ID mapping, DTC extraction, payload analysis, and practical steps for log analysis.                                |

Note: These documents are critical for anyone extending, debugging, or reverse-engineering the gateway. Always consult them before making changes to message handling or decoding logic.
