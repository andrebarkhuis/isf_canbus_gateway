...
struct SignalValue
{
    ...
    std::string parameter_name;
    double value;
    uint32_t raw_value; // Add raw_value to store the extracted raw data
    std::optional<std::string> display_value;
    std::optional<std::string> unit;

    // Constructor (without parameter_id, if you don’t need it)
    SignalValue(const std::string &name, double val, uint32_t raw,
                std::optional<std::string> display = std::nullopt,
                std::optional<std::string> unit_str = std::nullopt)
        : parameter_name(name), value(val), raw_value(raw), display_value(display), unit(unit_str) {}
};
